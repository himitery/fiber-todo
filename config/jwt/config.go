	...
	if cfg.ErrorHandler == nil {
		cfg.ErrorHandler = func(ctx fiber.Ctx, err error) error {
			if err.Error() == ErrJWTMissingOrMalformed.Error() {
				return ctx.Status(401).JSON(response.ErrorRes{
					Message: ErrJWTMissingOrMalformed.Error(),
				})
			}
			return ctx.Status(401).JSON(response.ErrorRes{
...
func multiKeyfunc(givenKeys map[string]keyfunc.GivenKey, jwkSetURLs []string) (gojwt.Keyfunc, error) {
...
func signingKeyFunc(key SigningKey) gojwt.Keyfunc {
